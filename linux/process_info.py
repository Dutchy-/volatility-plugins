...
class process_info(object):
    ...
    def _validate_registers(self):
        for reg in self.thread_registers:
            rip_ok = False
            rsp_ok = False

            for low, high in self._exec_maps_ranges:
                if low <= reg.rip <= high:
                    rip_ok = True
                    break

            for low, high in self.thread_stack_ranges:
                if low <= reg.rsp <= high:
                    rsp_ok = True
                    break

            stats['threads'] += 1
            if rip_ok and rsp_ok:
                stats['threads_reg_ok'] += 1
